...
import { toggleAccountTask } from "@/lib/tenant/account-tasks";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; taskId: string }> }) {
  try {
    ...
    return NextResponse.json(await toggleAccountTask(prisma, user, taskId, Body.parse(await req.json()).done));
